...
void cadastro(struct User userAtual, struct User users[20], int index){
    ...
    if(existe == 1){
        ...
        int categoria;
        while(count == 0){
            printf("|   Você deseja se cadastrar como cliente ou vendedor?  |\n( 1 ) Cliente\n( 2 ) Vendedor\n( 3 ) Voltar para o menu inicial\n> ");
            scanf("%d", &categoria);
            getchar(); //Soluciona erro de tipagem do input
            //SALVANDO CATEGORIA
            if(categoria == 1){
    ...
    while(count == 0){
        ...
        if(strcmp(name, userAtual.name) < 0 || strcmp(name, userAtual.name) > 0){
            ...
            if(escolha == 1){
                ...
                count=0;
                voltar(); //SE A PESSOA ESCOLHER NÃO SE CADATRAR O PROGRAMA TEM QUE VOLTAR PRA TELA INICIALL, COMO FAZ ISSO?????????????????
                break;
            }
            count++;
        }else{
